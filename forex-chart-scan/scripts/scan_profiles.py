...
from profiles import detect_aim_for_range_low
from seek_and_destroy import detect_seek_and_destroy


DEFAULT_EXCHANGE = "FOREXCOM"

MAJOR_FX_PAIRS: Sequence[Tuple[str, str]] = (
...
def _batched(items: Sequence[Tuple[str, str]], size: int) -> Iterable[Sequence[Tuple[str, str]]]:
...
def scan(
    ...
    for batch in _batched(MAJOR_FX_PAIRS, batch_size if batch_size > 0 else len(MAJOR_FX_PAIRS)):
        for symbol, exchange in batch:
            ...
            afr = detect_aim_for_range_low(df)
            rows.append(
                {
                    "symbol": symbol,
                    "seek_and_destroy": bool(snd.iloc[-1]) if len(snd) else False,
                    "aim_for_range_low": bool(afr.iloc[-1]) if len(afr) else False,
                }
            )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan FX pairs for Seek and Destroy and related profiles.")
